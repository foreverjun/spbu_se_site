...
def practice_index():
    ...
    if request.method == "POST":
        if request.form["read_button"]:
            ...
            notification = NotificationCoursework.query.filter_by(
                id=notification_id
            ).first()
    ...
    notifications = (
        NotificationCoursework.query.filter_by(recipient_id=user.id)
        .order_by(desc(NotificationCoursework.time))
        .all()
    )
...
def practice_guide():
    return render_template(
        "practice/student/guide.html", thesises=get_list_of_thesises()
    )
...
def practice_new_thesis():
    ...
    return render_template(
        "practice/student/new_practice.html",
        thesises=get_list_of_thesises(),
        user=user,
        review_filter=form,
        form=form,
...
def practice_choosing_topic():
    ...
    return render_template(
        "practice/student/choosing_topic.html",
        thesises=get_list_of_thesises(),
        form=form,
...
def practice_edit_theme():
    ...
    if request.method == "POST":
        if request.form["submit_button"] == "Сохранить":
            ...
            if not topic:
                ...
                db.session.commit()
                return redirect(
                    url_for("practice_choosing_topic", id=current_thesis_id)
                )
    ...
    return render_template(
        "practice/student/edit_theme.html",
        thesises=get_list_of_thesises(),
        form=form,
        practice=current_thesis,
    )
...
def practice_goals_tasks():
    ...
    if request.method == "POST":
        if (
            "submit_goal_button" in request.form
            and request.form["submit_goal_button"] == "Сохранить"
        ...
        if (
            "submit_goal_button" in request.form
            and request.form["submit_goal_button"] == "Сохранить изменения"
            and request.form["goal"]
        ):
            goal = request.form.get("goal", type=str)
        ...
        if "submit_task_button" in request.form:
            if "submit_task_button" in request.form:
                ...
                if same:
                    ...
                    new_task = ThesisTask(
                        task_text=task, current_thesis_id=current_thesis_id
                    )
    ...
    return render_template(
        "practice/student/goals_tasks.html",
        thesises=get_list_of_thesises(),
        practice=current_thesis,
        formGoal=formGoal,
        formTask=formTask,
...
def practice_workflow():
    ...
    return render_template(
        "practice/student/workflow.html",
        thesises=get_list_of_thesises(),
        practice=current_thesis,
        reports=reports,
    )
...
def practice_add_new_report():
    ...
    return render_template(
        "practice/student/new_report.html",
        thesises=get_list_of_thesises(),
        practice=current_thesis,
...
def practice_preparation():
    ...
    if request.method == "POST":
        if "submit_text_button" in request.form:
            ...
            if not current_thesis.text_uri and current_thesis.text_link:
                if text_file.filename:
                    if text_file and not allowed_file(text_file.filename):
                        ...
                        if text_file:
                            author_en = translit(
                                current_user.get_name(), "ru", reversed=True
                            )
                            author_en = author_en.replace(" ", "_")
                            text_filename = (
                                author_en
                                + "_"
                                + get_thesis_type_id_string(current_thesis.worktype_id)
                            )

                            text_filename = (
                                text_filename + "_" + str(date.today().year) + "_text"
                            )
                            ...
                            if os.path.isfile(full_text_filename):
                                text_filename = (
                                    text_filename + "_" + str(os.urandom(8).hex())
                                )
                                text_filename_with_ext = text_filename + ".pdf"
                                full_text_filename = os.path.join(
                                    TEXT_UPLOAD_FOLDER + text_filename_with_ext
            ...
            if (
                supervisor_review_file.filename == ""
                and consultant_review_file.filename == ""
            ):
                flash("Вы не загрузили отзыв.", category="error")
            elif (
                supervisor_review_file
                and not allowed_file(supervisor_review_file.filename)
                or consultant_review_file
                and not allowed_file(consultant_review_file.filename)
            ):
                ...
                if supervisor_review_file:
                    ...
                    full_review_filename = os.path.join(
                    ...
                    if os.path.isfile(full_review_filename):
                        review_filename = (
                            review_filename + "_" + str(os.urandom(8).hex())
                        )
                        review_filename_with_ext = review_filename + ".pdf"
                        full_review_filename = os.path.join(
                            REVIEW_UPLOAD_FOLDER + review_filename_with_ext
            ...
            if not current_thesis.presentation_uri and current_thesis.presentation_link:
                if presentation_file.filename:
                    if presentation_file and not allowed_file(
                        presentation_file.filename
                    ):
                        flash(
                            "Презентация должна быть в формате .PDF", category="error"
                        )
                    else:
                        if presentation_file:
                            ...
                            presentation_filename_with_ext = (
                                presentation_filename + ".pdf"
                            )
                            ...
                            current_thesis.presentation_uri = (
                                presentation_filename_with_ext
                            )
                            db.session.commit()
                ...
                current_thesis.presentation_uri and not current_thesis.presentation_link
            ):
                ...
                not current_thesis.presentation_uri
                and not current_thesis.presentation_link
            ):
                if (
                    not request.form["presentation_link"]
                    and not presentation_file.filename
                ):
                    flash(
                    ...
                    if presentation_file.filename:
                        if presentation_file and not allowed_file(
                            ...
                            if presentation_file:
                                ...
                                db.session.commit()
                                flash(
                                    "Презентация успешно загружена!", category="success"
                                )
    ...
    return render_template(
        "practice/student/preparation.html",
        thesises=get_list_of_thesises(),
        practice=current_thesis,
        deadline=deadline,
...
def practice_data_for_practice():
    ...
    return render_template(
        "practice/student/data_for_practice.html",
        thesises=get_list_of_thesises(),
        user=user,
        form=form,
        practice=current_thesis,
